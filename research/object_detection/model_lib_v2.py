...
def train_loop(
    ...
    **kwargs,
    ...
    """
    mlflow.log_artifact(pipeline_config_path)
    ...
    if summary_file_path is None:
        summary_file_path = os.path.join(model_dir, "train")

    summary_writer_filepath = get_filepath(
        strategy, os.path.join(summary_file_path, "train")
    )
    ...
    summary_writer_eval = tf.compat.v2.summary.create_file_writer(
        os.path.join(summary_file_path, "eval")
    )
        ...
        with strategy.scope():
            with tf.compat.v2.summary.record_if(
                    ...
                    if (
                        ...
                        if tracking_config:
                            log_metrics(
                                ...
                                config_metrics=tracking_config.metricas,
                            )
                            log_metrics(
                                name_dataset="val",
                                metrics_dict_np=metrics_valid_dict,
                ...
                (
                    metrics_valid_dict,
                    metrics_train_dict,
                ) = calculate_final_metrics_losses(
                    configs,
                    model_config,
                    train_input_config,
                    strategy,
...
def calculate_final_metrics_losses(
    ...
    eval_input_config.tf_record_input_reader.ClearField("input_path")
    eval_input_config.tf_record_input_reader.input_path.append(
        train_input_config.tf_record_input_reader.input_path[0]
    )

    train_eval_input = strategy.experimental_distribute_dataset(
        inputs.eval_input(
            eval_config=eval_config,
            eval_input_config=eval_input_config,
            model_config=model_config,
    ...
    # return metrics_valid_dict, metrics_train_dict, metrics_test_dict
    return metrics_valid_dict, metrics_train_dict
...
def save_results(
    ...
    if tracking_config:
        ...
        log_metrics(
            name_dataset="train",
            metrics_dict_np=losses_train_dict_np,
            config_metrics=tracking_config.losses,
        )

        log_metrics(
            name_dataset="val",
            metrics_dict_np=metrics_valid_dict_np,
            config_metrics=tracking_config.metricas,
        )

        log_metrics(
            name_dataset="val",
            metrics_dict_np=losses_valid_dict_np,
            config_metrics=tracking_config.losses,
        )
    ...
    if performance_summary_exporter is not None:
        metrics = {
            ...
            "train_metrics": metrics_train_dict_np,
            "validation_metrics": metrics_valid_dict_np,
...
def log_params(configs, params):
    ...
    for param in params:
        ...
        first_field = param_fields.pop(0)
        message = configs[first_field]
        for field in param_fields:
            _validate_message_has_field(message, field)
            message = getattr(message, field)

        mlflow.log_param(field, message)


def log_metrics(name_dataset, metrics_dict_np, config_metrics):
    for name_metric, metric in config_metrics.items():
        mlflow.log_metric(f"{name_dataset}/{name_metric}", metrics_dict_np[metric])
...
def convert_metrics(metrics_dict):
    metrics_dict_np = {
        (name): (
            ...
            else value.numpy().tolist()
        )
        for name, value in metrics_dict.items()
    }

    return metrics_dict_np
...
def eval_continuously(
    ...
    **kwargs,
    ...
    for latest_checkpoint in tf.train.checkpoints_iterator(
        ...
        if summary_file_path is None:
            summary_file_path = os.path.join(model_dir, "eval", eval_input_config.name)
        summary_writer = tf.compat.v2.summary.create_file_writer(summary_file_path)
